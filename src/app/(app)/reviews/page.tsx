...
export default async function ReviewsPage({
  ...
  return (
    <main className="mx-auto flex w-full max-w-6xl flex-col gap-6 px-6 py-8">
      ...
      {reviews.length === 0 ? (
        <section className="flex min-h-80 flex-col items-center justify-center rounded-md border border-dashed bg-canvas p-10 text-center">
          <div className="mb-4 flex h-12 w-12 items-center justify-center rounded-md border bg-canvas-subtle">
        ...
        <section className="space-y-4">
          ...
          <div className="overflow-hidden rounded-md border bg-canvas">
            <div className="grid grid-cols-[minmax(0,1fr)_180px_120px_130px_150px] border-b bg-canvas-subtle px-4 py-3 text-sm font-medium text-fg-muted">
              <span>Review</span>
              <span>Branch</span>
              <span>Status</span>
            ...
            {reviews.map((review) => (
              <Link
                key={review.id}
                className="grid grid-cols-[minmax(0,1fr)_180px_120px_130px_150px] border-b px-4 py-4 transition-colors last:border-b-0 hover:bg-canvas-subtle focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
              ...
              >
                <span>
                  ...
                  {review.prUrl ? (
                    <span
                      className="ml-2 inline-flex items-center gap-1 rounded-full border px-2 py-0.5 text-xs text-fg-muted"
                      title={review.prUrl}
                    >
                      PR #{review.prNumber ?? "?"}
